...
namespace Natom.AccessMonitor.Services.Mailer.Services
{
    public class MailService
    {
        ...
        public async Task EnviarEmailParaConfirmarRegistroAsync(Transaction transaction, string scope, Usuario usuario)
        {
            ...
            body = body.Replace("$body$", String.Format("<h2>¡Bienvenido a BioAnviz+!</h2><br/><p>Por favor, para <b>generar la clave de acceso al sistema</b> haga clic en el siguiente link: <a href='{0}'>{0}</a></p>", link));
        ...
        public async Task EnviarEmailParaRecuperarClaveAsync(Transaction transaction, string scope, Usuario usuario)
        {
            ...
            string link = new Uri($"{appAddress}/users/confirm/{data}").AbsoluteUri;
            string body = ResourceTemplates.Default;
            body = body.Replace("$body$", String.Format("<h2>Recupero de clave BioAnviz+</h2><br/><p>Por favor, para <b>recuperar la clave de acceso al sistema</b> haga clic en el siguiente link: <a href='{0}'>{0}</a></p>", link));
            await EnviarMailAsync(transaction, subject, body, usuario.Email, usuario.Nombre);
        }
